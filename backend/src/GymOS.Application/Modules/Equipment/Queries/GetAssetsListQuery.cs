...
public record GetAssetsListQuery(Guid? BranchId, AssetStatus? Status, string? Category, int Page = 1, int PageSize = 20)
    : IQuery<PagedList<AssetListItemDto>>;

public class GetAssetsListQueryHandler(IApplicationDbContext db, ICurrentUserService currentUser)
    : IRequestHandler<GetAssetsListQuery, PagedList<AssetListItemDto>>
{
    public async Task<PagedList<AssetListItemDto>> Handle(GetAssetsListQuery request, CancellationToken cancellationToken)
    {
