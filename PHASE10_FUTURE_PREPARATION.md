...
| **Historical analytics** | Good foundation | All member activity is append-only and dated; measurements are never overwritten (each is a new row). Aggregate reporting now exists for all 10 report-bearing domains, including Workouts (most-logged exercises, sets/reps/avg weight) and Nutrition (most-logged food items, calorie/water summary) — closed after this document's original Phase 8 Foundation-gap note. |
